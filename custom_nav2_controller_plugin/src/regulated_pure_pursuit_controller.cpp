...
void RegulatedPurePursuitController::configure(
  ...
  declare_parameter_if_not_declared(
      node, plugin_name_ + ".use_approach_linear_velocity_scaling",
  ...
  node->get_parameter(plugin_name_ + ".use_approach_linear_velocity_scaling",
                      use_approach_vel_scaling_);
  node->get_parameter(plugin_name_ + ".max_allowed_time_to_collision",
...
double RegulatedPurePursuitController::costAtPose(const double &x,
  ...
  if (!costmap_->worldToMap(x, y, mx, my)) {
    RCLCPP_FATAL(logger_, "The dimensions of the Costmap are too small to "
                          "fully include your robot's footprint,"
                          "thusly the robot cannot proceed further");
    throw nav2_core::PlannerException(
        "RegulatedPurePursuitController: Dimensions of the Costmap are too "
        "small to encapsulate the robot footprint at current speeds!");
  }
...
void RegulatedPurePursuitController::applyConstraints(
  ...
  // we're at the end of the path. You will scale linear velocity by error to
  // slow to a smooth stop. This expression is eq. to (1) holding time to goal,
  // t, constant using the theoretical lookahead distance and proposed velocity
  // and (2) using t with the actual lookahead distance to scale the velocity
  // (e.g. t = lookahead / velocity, v = carrot / t).
...
nav_msgs::msg::Path RegulatedPurePursuitController::transformGlobalPlan(
  ...
  // Find points definitely outside of the Costmap so you will not transform
  // them.
  ...
  // Remove the portion of the global plan that you have already passed so we do
  // not process it on the next iteration (this is called path pruning)
...
double RegulatedPurePursuitController::findDirectionChange(
  ...
  for (unsigned int pose_id = 1; pose_id < global_plan_.poses.size() - 1;
       ++pose_id) {
    // You have two vectors for the dot product OA and AB. Determining the
    // vectors.
...
PLUGINLIB_EXPORT_CLASS(custom_nav2_controller_plugin_namespace_name::
