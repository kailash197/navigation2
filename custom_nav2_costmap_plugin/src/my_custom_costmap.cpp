...
void MyCustomCostmap::onInitialize() {
...
// update. Inside this method window bounds are re-calculated if
// need_recalculation_ is true and updated independently on its value.
void MyCustomCostmap::updateBounds(double /*robot_x*/, double /*robot_y*/,
...
void MyCustomCostmap::onFootprintChanged() {
  need_recalculation_ = true;

  RCLCPP_DEBUG(
      rclcpp::get_logger("nav2_costmap_2d"),
      "MyCustomCostmap::onFootprintChanged(): num footprint points: %lu",
...
// layers.
void MyCustomCostmap::updateCosts(nav2_costmap_2d::Costmap2D &master_grid,
  ...
  // In this case, using master_array pointer is equal to modifying local
  // costmap_ pointer and then calling updateWithTrueOverwrite():
  ...
  RCLCPP_WARN(rclcpp::get_logger("nav2_costmap_2d"), "circle_p_x = %f",
              circle_p_x);
  RCLCPP_WARN(rclcpp::get_logger("nav2_costmap_2d"), "circle_p_y = %f",
              circle_p_y);
...
bool MyCustomCostmap::pointInsideCircle(int x_point, int y_point) {
...
// This is the macro allowing a
// my_custom_costmap_namespace_name::MyCustomCostmap class to be registered in
// order to be dynamically loadable of base type nav2_costmap_2d::Layer. Usually
// places in the end of cpp-file where the loadable class written.
#include "pluginlib/class_list_macros.hpp"
PLUGINLIB_EXPORT_CLASS(my_custom_costmap_namespace_name::MyCustomCostmap,
